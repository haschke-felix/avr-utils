...
{
public:
	explicit StaticList(const unsigned int max_size_);
	~StaticList();
...
template <class T>
StaticList<T>::StaticList(const unsigned int size) : max_size_(size), size_(0)
{
...
template <class T>
T &StaticList<T>::itemAt(const unsigned int index) const
{
...
{
	for (; size_; --size_) first_[size_] = T();
}
